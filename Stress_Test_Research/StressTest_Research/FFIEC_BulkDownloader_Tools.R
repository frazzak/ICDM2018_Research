...
selected_val = select[which(selected == "selected")]
select
...
#Get Tables for to walk through all BHCs
  ...
  topBHC$`12/31/2017 Total Assets` = as.numeric(str_replace_all(topBHC$`12/31/2017 Total Assets`, "[^[:alnum:]]", ""))


  
#Update Column Names
  names(topBHC)[3] = 'Total_Assets'
#Change webpage using dropdown.
  summary(topBHC)  
...
BHCWebAPILinks = FFIEC_BHC_WebAPI_UrlCreator(RSSD_IDs[1],ReportDates,ReportTypes,downloadpath)
View(head(BHCWebAPILinks))


DownloadBHCLog = WebAPI_URLLink_Downloader(BHCWebAPILinks[1:6,])
View(DownloadBHCLog)
